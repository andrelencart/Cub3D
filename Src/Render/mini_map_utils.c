...
void	mini_map_vision_draw(t_cube *cube, int draw_x, int draw_y)
{
	...
	while (distance < LIGHT_RAD)
	{
		...
		if (is_wall(cube, ray_x, ray_y))
			break ;
		draw_x = (int)(cube->mini_map.player_mini_x + cube->mini_map.ray_dir_x * distance * MINIMAP_TILE_SIZE);
		draw_y = (int)(cube->mini_map.player_mini_y + cube->mini_map.ray_dir_y * distance * MINIMAP_TILE_SIZE);
...
void vision_mini_map_init(t_cube *cube, int half_view)
{
	...
	cube->mini_map.light.player_angle = atan2(cube->player.dir_y, cube->player.dir_x);
	cube->mini_map.light.start_angle = cube->mini_map.light.player_angle - cube->mini_map.light.m_m_fov / 2;
	cube->mini_map.light.end_angle = cube->mini_map.light.player_angle + cube->mini_map.light.m_m_fov / 2;
}