...
int	get_data(char *fname, t_parse *data)
{
	...
	if (!line)
		return (parse_error("get_data: Failed to allocate *line"));
	fd = open(line, O_RDONLY);
	if (fd < 0)
		return (free(line), parse_error(strerror(errno)));
	while (line != NULL)
	{
		if (i == 0)
			free(line);
...
int	find_file(char *fname, t_parse *data)
{
	...
	line = ft_strjoin("maps/", fname);
	if (!line)
		return (parse_error("find_file: Failed to allocate *line"));
	fd = open(line, O_RDONLY);
	if (fd < 0)
		return (free(line), parse_error(strerror(errno)));
	while (line != NULL)
	...
	if (!data->map)
		return (parse_error("find_file: Failed to allocate data->map"));
...
int	parse_check_fname(char *fname, t_parse *data)
{
	...
	if (len < 5 || ft_strncmp(&fname[len - 4], ".cub", 4))
		return (parse_error("File name is not valid"));
